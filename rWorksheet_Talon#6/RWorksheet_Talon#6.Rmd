...
#Each level follows a state or region and is arranged alphabetically.
...
meanIn <- tapply(incomes, stateFactor, mean)
meanIn
...
The output has the mean of income for each state.
...
standErr <- tapply(incomes, stateFactor, stdError)
standErr

```

b. Interpret the results.
The state's error are estimated mean income which is in the vector incster.
The variability of the mean is shown in the standard error.


7. Use the titanic dataset.

a. subset the titanic dataset of those who survived and not survived. Show the codes
and its result.
...
buhi <- subset(titanic_data, Survived == 1)


ratioed <- subset(titanic_data, Survived == 0)
ratioed
...
d.1 Standard error of the mean for clump thickness.insarisnat
